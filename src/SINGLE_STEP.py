...
CONFIG = {
    ...
    "model_save_path": "saved_models",   # Ordner für gespeicherte Modelle
    "selected_model": "LSTM",            # Optionen: "LSTM", "MLP", "GRU", "AttentionLSTM"
    
    # Trainings-Hyperparameter
    "max_epochs": 25,
    "batch_size": 256,
    "learning_rate": 0.006416042484848492,
    "hidden_size": 192,
    "num_layers": 2,
    "dropout": 0.2,
    ...
    # Zeit-Parameter
    "training_weeks": 52,     # Wochen an historischen Daten für das Training
    "sequence_length": 24,    # Länge des Input-Fensters
    "forecast_horizon": 168,  # Prognosehorizont in Stunden
    
    # Scaler-Auswahl
    "scaler_type": "MinMax",  # Optionen: "Standard", "MinMax", oder None
...
def load_and_prepare_data(config):
    # Ändere "ready_for_MODEL.pkl" ggf. auf Deinen Dateinamen.
...
##########################
#   SINGLE-STEP FUNKTIONEN
##########################
def train_and_forecast_single_step(config, forecast_start_date):
    """
    Trainiert ein Modell auf historischen Daten (bis forecast_start_date) und 
    macht Vorhersagen über config['forecast_horizon'].
    ...
    Liefert zusätzlich ein Dictionary mit den Metriken zurück.
    """
    ...
    train_df = df_scaled.iloc[idx_train_start:idx_start]
    data_features = train_df[config["feature_columns"]].values
    data_targets = train_df[config["target_column"]].values
    
    dataset = TimeSeriesDataset(data_features, data_targets,
    ...
    for epoch in range(config["max_epochs"]):
        ...
        if config["verbose"]:
            print(f"[Single-Step] Epoch {epoch+1}/{config['max_epochs']} - Train Loss: {train_loss:.4f}")
    
    seq_len = config["sequence_length"]
    horizon = config["forecast_horizon"]
    
    start_idx_for_df = max(0, idx_start - seq_len)
    ...
    for i in range(idx_start, min(idx_start + horizon, len(df_scaled))):
        if i - seq_len < 0:
            continue
        window_data = df_scaled.iloc[i - seq_len: i][config["feature_columns"]].values
    ...
    if len(preds_series) > 0:
        ...
        print(f"[Single-Step] Forecast Metrics: MSE={mse:.4f}, MAE={mae:.4f}, R2={r2:.4f}")
    else:
        print("[Single-Step] Hinweis: Keine Vorhersagen im gegebenen Zeitraum!")
    ...
    model_file = os.path.join(config["model_save_path"], "my_single_step_model.pt")
    torch.save({
    ...
    print(f"[Single-Step] Modell gespeichert unter: {model_file}")
    
    return df_result, metrics


def load_and_forecast_single_step(config, forecast_start_date):
    """
    Lädt ein gespeichertes Modell und prognostiziert ab forecast_start_date über forecast_horizon.
    Erzeugt ein DataFrame für den Zeitraum [start_date - sequence_length, start_date + forecast_horizon]
    und gibt zusätzlich ein Dictionary mit den Metriken zurück.
    """
    df_scaled, scaler_X, scaler_y = load_and_prepare_data(config)
    idx_start = df_scaled.index.get_loc(forecast_start_date)
    ...
    model = create_model(model_type, input_dim, hidden_size, num_layers, dropout,
                         sequence_length=config["sequence_length"])
    ...
    if len(preds_series) > 0:
        ...
        r2 = r2_score(real_series, preds_series)
        metrics = {"mse": mse, "mae": mae, "r2": r2}
        print(f"[Single-Step Load] Forecast Metrics: MSE={mse:.4f}, MAE={mae:.4f}, R2={r2:.4f}")
    else:
        print("[Single-Step Load] Keine Vorhersagen im gegebenen Zeitraum!")
    ...
    if config.get("show_plots", True):
        ...
        plt.plot(df_result.index, df_result["prediction"], label="Prediction", marker='x')
        plt.xticks(rotation=45)
        plt.title("Single-Step Forecast (Geladenes Modell)")
        plt.legend()
        plt.tight_layout()
        plt.show()
...
if __name__ == "__main__":
    example_date = "2025-04-01 00:00:00"
    df_result, metrics = train_and_forecast_single_step(CONFIG, forecast_start_date=example_date)
    df_result.to_pickle("df_single_step_result.pkl")
    
    loaded_df, load_metrics = load_and_forecast_single_step(CONFIG, forecast_start_date=example_date)
    loaded_df.to_pickle("df_single_step_loaded.pkl")
